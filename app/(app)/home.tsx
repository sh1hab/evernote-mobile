...
// const installedModules = async () => {
//   try {
//     const value = await AsyncStorage.getItem('installedModules');
//     if (value !== null) {
//       console.log(value);
//     }
//   } catch (error) {
...
const ModularHomePage = () => {
  ...
  const installModule = (module) => {
    ...
    if (!installedModules.find(m => m.id === module.id)) {
      ...
      AsyncStorage.setItem('installedModules', JSON.stringify([...installedModules, { ...module, installed: true }]));
    }
  ...
  const ModuleCard = ({ module }) => (
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        console.log('module', module.name.toLowerCase());
        try {
          router.push('/(app)/' + module.name.toLowerCase());
        }
        catch (exception) {
